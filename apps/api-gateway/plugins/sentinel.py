...
def register(app, fusion_url: str):
    """Register Sentinel routes on the API Gateway."""
    router = APIRouter()

    @router.get("/v1/sentinel/ignitions")
    async def get_ignitions(limit: int = 50):
        # Convention: ignitions are observations with class 'fire.ignition'
