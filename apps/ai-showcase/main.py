...
async def startup_event():
    ...
    fusion_network = MultimodalFusionNetwork(
        input_dims={"weather": 5, "lidar": 1, "thermal": 1, "visual": 3}, hidden_dim=512
    )
...
async def demo_fire_prediction():
    ...
    prediction = fire_predictor.predict_spread(
        fire_location, weather_data, terrain_data
    )

    # Add visualization data
    prediction["visualization"] = {
        "spread_pattern": generate_spread_pattern(
            fire_location, prediction["spread_rate"]
        ),
...
if __name__ == "__main__":
    uvicorn.run("main:app", host="0.0.0.0", port=8008, reload=True, log_level="info")
