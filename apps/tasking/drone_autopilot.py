        ...
        try:
            ...
            for wp in waypoints:
                ...
                if action in (
                    ...
                    hold_ms = int(
                        params.get("hold_ms", params.get("activation_ms", 0)) or 0
                    )
                    if hold_ms > 0:
                        mission_items.append(
                            {
            ...
            logger.info(
                f"Set mission with {len(mission_items)} items ({len(waypoints)} nav points) for drone {self.device_id}"
            )
            return True
        ...
        while self.connected:
            try:
                ...
                if (
                    ...
                    > self.emergency_procedures["connection_timeout"]
                ):
                ...
                if (
                    self.gps_loss_start
                    and (current_time - self.gps_loss_start)
                    > self.emergency_procedures["gps_loss_threshold"]
                ):
...
class FireFlyFleetManager:
    ...
    def get_fleet_status(self) -> Dict[str, Any]:
        ...
        return {
            firefly_id: firefly.get_status()
            for firefly_id, firefly in self.fireflies.items()
        }
