...
import { fetchMissions, fetchAlerts, connectWebSocket, MissionAPI, AlertAPI } from '@/lib/api';
...
function ResourceStatus() {
  ...
  useEffect(() => {
    fetchMissions(20).then((data) => setMissions(data || [])).catch(() => {});
    fetchAlerts(100).then((r) => setAlerts(r.alerts || [])).catch(() => {});
  }, []);
...
export default function CommandLayout({ onSwitchRole }: CommandLayoutProps) {
  ...
  useEffect(() => {
    fetchMissions(20)
      .then((data) => {
        setMissions(data || []);
        const pending = (data || []).filter((m) => m.status.toUpperCase() === 'PENDING_APPROVAL').length;
  ...
  return (
    <div className="fixed inset-0 flex flex-col" style={{ background: '#080C0A' }}>
      {/* Top bar */}
      <CommandTopBar onSwitchRole={onSwitchRole} pendingApprovals={pendingApprovals} />
