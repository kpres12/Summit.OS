...
class SmokeDetectionCNN(nn.Module):
    ...
    def __init__(self, input_channels: int = 3, num_classes: int = 2):
        ...
        self.attention = nn.Sequential(
            nn.Conv2d(128, 64, 1), nn.ReLU(), nn.Conv2d(64, 1, 1), nn.Sigmoid()
        )
...
class FireDetectionEngine:
    ...
    def detect_fire(
        self, image: np.ndarray, thermal_data: Optional[np.ndarray] = None
    ) -> List[FireDetection]:
        """
    ...
    def analyze_fire_spread(
        self, detections: List[FireDetection], environmental_data: Dict[str, Any]
    ) -> FireAnalysis:
        """
        ...
        try:
            ...
            risk_level = self._calculate_risk_level(
                detections, spread_rate, probability
            )
    ...
    def _find_attention_peaks(
        self, attention_map: np.ndarray, threshold: float = 0.5
    ) -> List[Tuple[int, int]]:
        """Find peaks in attention map"""
    ...
    def _estimate_bbox(
        self, center_x: float, center_y: float, image_shape: Tuple[int, ...]
    ...
    def _filter_false_positives(
        self, detections: List[FireDetection], image: np.ndarray
    ) -> List[FireDetection]:
    ...
    def _enhance_with_thermal(
        self, detections: List[FireDetection], thermal_data: np.ndarray
    ) -> List[FireDetection]:
    ...
    def _calculate_risk_level(
        self, detections: List[FireDetection], spread_rate: float, probability: float
    ) -> str:
    ...
    def _calculate_containment_priority(
        self,
        detections: List[FireDetection],
        spread_rate: float,
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fire_spread_predictor": {
                "parameters": sum(
                    p.numel() for p in self.fire_spread_predictor.parameters()
                ),
                "device": str(self.device),
            },
