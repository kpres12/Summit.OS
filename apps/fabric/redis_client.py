...
class RedisClient:
    ...
    async def add_telemetry(self, telemetry: TelemetryMessage):
        ...
        try:
            ...
            stream_data = {
                ...
                "battery_level": (
                    str(telemetry.battery_level) if telemetry.battery_level else ""
                ),
                "signal_strength": (
                    str(telemetry.signal_strength) if telemetry.signal_strength else ""
                ),
            ...
            try:
                await self.redis.xtrim(
                    "telemetry_stream", maxlen=100000, approximate=True
                )
            except Exception:
                pass
    ...
    async def add_alert(self, alert: AlertMessage):
        ...
        try:
            ...
            # Store alert details
            await self.redis.hset(f"alert:{alert.alert_id}", mapping=stream_data)
    ...
    async def add_mission_update(self, mission: MissionUpdate):
        ...
        try:
            ...
            await self.redis.hset(f"mission:{mission.mission_id}", mapping=stream_data)
    ...
    async def add_operation_event(self, event: Dict[str, Any]):
        ...
        try:
            ...
            try:
                await self.redis.xtrim(
                    "operations_stream", maxlen=50000, approximate=True
                )
            except Exception:
                pass
        except Exception as e:
    ...
    async def process_telemetry_stream(self):
        ...
        try:
            ...
            messages = await self.redis.xread(
                {"telemetry_stream": "$"}, count=100, block=1000
            )
    ...
    async def process_alert_stream(self):
        ...
        try:
            ...
            messages = await self.redis.xread(
                {"alert_stream": "$"}, count=100, block=1000
            )
