...
class OfflineDataManager:
    ...
    async def store_message(
        self, message_type: str, data: Dict[str, Any], priority: int = 1
    ) -> str:
    ...
    async def mark_message_failed(
        self, message_id: str, increment_retry: bool = True
    ) -> bool:
...
class OfflineManager:
    ...
    async def store_telemetry(
        self, telemetry_data: Dict[str, Any], priority: int = 1
    ) -> str:
        ...
        return await self.data_manager.store_message(
            "telemetry", telemetry_data, priority
        )
            ...
            if self.entity_crdt and self.summit_client:
                try:
                    ...
                    if sync_payload and sync_payload.get("entities"):
                        ...
                        logger.info(
                            f"Syncing {len(sync_payload['entities'])} CRDT entities"
                        )
                except Exception as e:
