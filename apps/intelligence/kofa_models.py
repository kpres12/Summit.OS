...
class KofaModels:
    ...
    def __init__(self) -> None:
        ...
        n = sum(1 for s in [
            self._fp_filter, self._escalation, self._correlator,
            self._weather_risk, self._outcome, self._asset_assign, self._seq_anomaly,
        ] if s is not None)
        ...
        try:
            ...
            for recent in reversed(list(self._recent_obs)):
                ...
                pair_feat = _make_pair_features(
                    obs, recent["obs"], feat_a, recent["feat"],
                    lat_a, lon_a, recent["lat"], recent["lon"],
                    conf_a, float(recent["obs"].get("confidence") or 0), age,
                )
                ...
                if same_prob >= self.CORRELATE_THRESHOLD:
                    logger.info(
                        ...
                        same_prob * 100, age,
                        obs.get("class"), recent["obs"].get("class"),
                    )
                    return True
            return False
        except Exception as exc:
    ...
    def adjust_risk_for_weather(self, obs: dict, base_risk: str) -> str:
        ...
        _WEATHER_FIELDS = [
            "wind_speed_mps", "wind_gust_mps", "humidity_pct",
            "temp_c", "precip_mm", "visibility_km",
        ]
...
def _make_pair_features(
    ...
    feat_a: List[float], feat_b: List[float],
    ...
    _DOMAIN_MAP = {
        "fire": 0, "smoke": 0, "flame": 0, "wildfire": 0, "ember": 0,
        "flood": 1, "surge": 1, "inundation": 1, "tsunami": 1,
        "person": 2, "missing": 2, "survivor": 2, "casualty": 2,
        "hazmat": 3, "chemical": 3, "spill": 3, "radiation": 3,
        "collapse": 4, "rubble": 4, "earthquake": 4, "landslide": 4,
    }
    _COPAIRS = [
        ("fire", "smoke"), ("flood", "surge"), ("flood", "inundation"),
        ("person", "missing"), ("earthquake", "collapse"), ("chemical", "plume"),
    ]
...
def _build_sequence_features(history: List[dict]) -> List[float]:
    ...
    h_changes = [min(abs(headings[i+1] - headings[i]), 360 - abs(headings[i+1] - headings[i]))
                 for i in range(n - 1)]
    mean_hc  = sum(h_changes) / len(h_changes) if h_changes else 0.0
    hc_std   = math.sqrt(sum((c - mean_hc) ** 2 for c in h_changes) / len(h_changes)) if h_changes else 0.0
    ...
    mean_lon = sum(lons) / n
    pos_var  = math.sqrt(
        sum(((la - mean_lat) * 111000) ** 2 + ((lo - mean_lon) * 111000) ** 2
            for la, lo in zip(lats, lons)) / n
    )
