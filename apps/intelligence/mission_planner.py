...
@dataclass
class MissionPlan:
    mission_type: str        # SURVEY | MONITOR | SEARCH | PERIMETER | ORBIT | DELIVER | INSPECT
    lat: float
    lon: float
...
_RULES: List[Dict] = [
    # Fire / smoke
    {
        "keywords": ["smoke", "fire", "flame", "wildfire", "ember", "hotspot", "burning", "blaze"],
        "confidence_min": 0.55,
        "mission_type": "SURVEY",
        "alt_m": 120, "asset_class": "UAV", "loiter": True,
        "rationale_template": "{cls} detected — aerial survey to confirm extent",
    },
    # Person / survivor
    {
        "keywords": ["person", "human", "survivor", "victim", "pedestrian", "casualty",
                     "injured", "missing", "stranded", "civilian", "hiker", "swimmer"],
        "confidence_min": 0.60,
        "mission_type": "MONITOR",
        "alt_m": 60, "asset_class": "UAV", "loiter": True,
        "rationale_template": "{cls} detected — continuous monitoring for response coordination",
    },
    # Search targets (lost/downed/mayday)
    {
        "keywords": ["missing", "lost", "overdue", "distress", "sos", "mayday", "downed", "epirb", "plb"],
        "confidence_min": 0.45,
        "mission_type": "SEARCH",
        "alt_m": 80, "asset_class": "UAV", "loiter": False,
        "rationale_template": "{cls} — systematic search pattern",
    },
    # Flood / water
    {
        "keywords": ["flood", "inundation", "surge", "submerged", "tsunami", "levee breach", "rising water"],
        "confidence_min": 0.60,
        "mission_type": "SURVEY",
        "alt_m": 200, "asset_class": "FIXED_WING", "loiter": False,
        "rationale_template": "{cls} event — wide-area survey for extent mapping",
    },
    # Structural damage
    {
        "keywords": ["collapse", "rubble", "debris", "landslide", "mudslide", "sinkhole", "destroyed"],
        "confidence_min": 0.55,
        "mission_type": "SEARCH",
        "alt_m": 50, "asset_class": "UAV", "loiter": False,
        "rationale_template": "{cls} — search pattern for trapped survivors",
    },
    # Hazmat / chemical
    {
        "keywords": ["hazmat", "chemical", "spill", "leak", "toxic", "gas", "radiation",
                     "explosion", "contamination", "plume", "biological"],
        "confidence_min": 0.50,
        "mission_type": "PERIMETER",
        "alt_m": 150, "asset_class": "UAV", "loiter": False,
        "rationale_template": "{cls} — perimeter for exclusion zone",
    },
    # Security / intrusion
    {
        "keywords": ["intrusion", "unauthorized", "trespass", "breach", "armed", "hostile", "perimeter violation"],
        "confidence_min": 0.55,
        "mission_type": "PERIMETER",
        "alt_m": 100, "asset_class": "UAV", "loiter": True,
    ...
    {
        "keywords": ["power line", "pipeline", "bridge", "tower", "dam", "rail",
                     "pylon", "transformer", "substation", "antenna", "solar", "turbine"],
        "confidence_min": 0.55,
        "mission_type": "INSPECT",
        "alt_m": 30, "asset_class": "UAV", "loiter": False,
        "rationale_template": "{cls} — close-proximity inspection",
    },
    # Delivery / logistics
    {
        "keywords": ["delivery", "drop zone", "supply drop", "aid delivery", "package", "cargo", "payload"],
        "confidence_min": 0.70,
    ...
    # Agricultural
    {
        "keywords": ["crop", "field", "farm", "orchard", "irrigation", "pest", "harvest", "blight"],
        "confidence_min": 0.55,
        "mission_type": "SURVEY",
        "alt_m": 60, "asset_class": "UAV", "loiter": False,
...
_DEFAULT_RULE: Dict = {
    "mission_type": "SURVEY",
    "alt_m": 100, "asset_class": "UAV", "loiter": False,
...
_TYPE_ALTS: Dict[str, float] = {
    "SURVEY": 100, "MONITOR": 60, "SEARCH": 80, "PERIMETER": 150,
    "ORBIT": 120, "DELIVER": 80, "INSPECT": 30,
}
_TYPE_ASSET: Dict[str, str] = {
    "SURVEY": "UAV", "MONITOR": "UAV", "SEARCH": "UAV", "PERIMETER": "UAV",
    "ORBIT": "UAV", "DELIVER": "UAV", "INSPECT": "UAV",
}
_LABEL_MAP_INV: Dict[str, int] = {
    "SURVEY": 0, "MONITOR": 1, "SEARCH": 2, "PERIMETER": 3,
    "ORBIT": 4, "DELIVER": 5, "INSPECT": 6,
}
...
class MissionPlanner:
    ...
    _DEFAULT_MODEL = os.path.normpath(
        os.path.join(os.path.dirname(__file__), "..", "..", "packages", "ml", "models", "mission_classifier.onnx")
    )
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        if path and os.path.exists(path) and _FEATURES_AVAILABLE:
            try:
                ...
                logger.info("MissionPlanner: ML model loaded (%s)", os.path.basename(path))
            except Exception as exc:
                logger.warning("MissionPlanner: ML model load failed (%s) — rules only", exc)
                self._session = None
    ...
    def plan(self, observation: Dict[str, Any]) -> Optional[MissionPlan]:
        ...
        ml_type: Optional[str] = (
            self._predict(observation) if (self._session and confidence >= 0.55) else None
        )
