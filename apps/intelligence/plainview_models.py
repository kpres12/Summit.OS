...
class FlowAnomalyDetector:
    ...
    def _detect_ml(self, metrics: Dict[str, float]) -> Dict[str, Any]:
        ...
        try:
            ...
            anomaly_type = (
                anomaly_types[int(outputs[1][0])] if len(outputs) > 1 else "unknown"
            )
    ...
    def _detect_rules(self, metrics: Dict[str, float]) -> Dict[str, Any]:
        ...
        # Flow rate check
        flow_dev = (
            abs(flow - self.baseline["flow_rate_lpm"]) / self.baseline["flow_rate_lpm"]
        )
        if flow_dev > 0.25:
...
class ValveHealthPredictor:
        ...
        try:
            ...
            risk_level = (
                ...
                if health_score < 30
                    ...
                    "high"
                    if health_score < 60
                    else "medium" if health_score < 80 else "low"
                )
            )

            recommendations = self._generate_recommendations(
                health_score, maintenance_days
            )
    ...
    def _generate_recommendations(
        self, health_score: float, maintenance_days: int
    ) -> List[str]:
...
class PipelineIntegrityAssessor:
        ...
        try:
            ...
            risk_level = (
                ...
                if leak_probability > 0.7
                    ...
                    else "medium" if leak_probability > 0.3 else "low"
                )
            )
    ...
    def _assess_rules(self, pipeline_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        risk_level = (
            "critical"
            if leak_prob > 0.6
            else "high" if leak_prob > 0.4 else "medium" if leak_prob > 0.2 else "low"
        )
...
def get_flow_detector() -> FlowAnomalyDetector:
    ...
    if _flow_detector is None:
        ...
        _flow_detector = FlowAnomalyDetector(
...
def get_valve_predictor() -> ValveHealthPredictor:
    ...
    if _valve_predictor is None:
        ...
        _valve_predictor = ValveHealthPredictor(
            model_path if os.path.exists(model_path) else None
        )
...
def get_pipeline_assessor() -> PipelineIntegrityAssessor:
    ...
    if _pipeline_assessor is None:
        ...
        _pipeline_assessor = PipelineIntegrityAssessor(
            model_path if os.path.exists(model_path) else None
        )
