...
def test_fire_dispatches_survey():
    ...
    p = MissionPlanner()
    plan = p.plan(
        {"class": "wildfire", "confidence": 0.80, "lat": 34.05, "lon": -118.24}
    )
    assert plan is not None
    assert plan.mission_type == "SURVEY"
...
def test_collapse_dispatches_search():
    ...
    p = MissionPlanner()
    plan = p.plan(
        {"class": "building collapse", "confidence": 0.75, "lat": 34.0, "lon": -118.0}
    )
    assert plan is not None
...
def test_hazmat_dispatches_perimeter():
    ...
    p = MissionPlanner()
    plan = p.plan(
        {"class": "chemical spill", "confidence": 0.70, "lat": 34.0, "lon": -118.0}
    )
    assert plan is not None
    assert plan.mission_type in (
        "PERIMETER",
        "MONITOR",
    )  # both defensible for hazmat containment
...
def test_low_confidence_unknown_returns_none():
    ...
    p = MissionPlanner()
    plan = p.plan(
        {"class": "unknown_thing", "confidence": 0.50, "lat": 34.0, "lon": -118.0}
    )
    assert plan is None
...
def test_high_confidence_unknown_dispatches():
    ...
    p = MissionPlanner()
    plan = p.plan(
        {"class": "alien_object", "confidence": 0.92, "lat": 34.0, "lon": -118.0}
    )
    assert plan is not None
