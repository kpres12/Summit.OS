    ...
    def test_waypoints_within_radius_approx(self):
        ...
        for sec in sectors:
            for wp in sec.waypoints:
                ...
                assert d <= RADIUS_M * 1.5, f"waypoint {wp} is {d:.0f}m from center (limit {RADIUS_M * 1.5:.0f}m)"
...
class TestSitRepGenerator:
    ...
    def test_sitrep_ids_unique(self):
        from sitrep import SitRepGenerator
        gen = SitRepGenerator()
        ids = {gen.from_advisories(self._make_advisories(2)).sitrep_id for _ in range(5)}
        assert len(ids) == 5
