...
class SitRepGenerator:
    ...
    def from_advisories(
        ...
        for adv in advisories:
            ...
            parts = msg.split(":")
            cls = (
                parts[1].strip().split(" detected")[0].strip()
                if len(parts) > 1
                else "unknown"
            )
        ...
        findings.sort(key=lambda f: (_SEV_ORDER.get(f.max_severity, 0) * -1, -f.count))
    ...
    async def enhance_with_llm(
        self, sitrep: SitRep, raw_advisories: List[Dict]
    ) -> SitRep:
        """
                ...
                if start >= 0 and end > start:
                    ...
                    if llm_summary:
                        ...
                        sitrep.recommended_action = (
                            llm_action or sitrep.recommended_action
                        )
...
def _make_summary(
    ...
    return (
        ...
        else f"Situation nominal. {count_str} in current window. "
