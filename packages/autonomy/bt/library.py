...
from packages.autonomy.bt.nodes import (
    ...
    Inverter, Repeat, RetryUntilSuccess, Timeout,
    Action, Condition, Wait, SetBlackboard, CheckBlackboard,
)
...
def build_patrol_tree(waypoints: List[Dict]) -> BehaviorTree:
    ...
    Structure:
      ...
      └── Sequence(RTB)
          └── Action(ReturnToBase)
    """
    root = Selector("PatrolMission", children=[
        Sequence("NormalOps", children=[
            Condition("BatteryOK", predicate=_check_battery),
            Sequence("Patrol", children=[
                Repeat("VisitWaypoints", count=len(waypoints), child=
                    Action("NavigateToWaypoint", action=_navigate_to_waypoint)
                ),
                Action("ReportPosition", action=_report_position),
            ]),
        ]),
        Sequence("RTB", children=[
...
def build_surveillance_tree(area_center: Dict, loiter_sec: float = 300) -> BehaviorTree:
    ...
    Structure:
      ...
      └── Action(RTB)
    """
    root = Selector("Surveillance", children=[
        Sequence("Mission", children=[
            Condition("BatteryOK", predicate=_check_battery),
            Action("NavigateToArea", action=_navigate_to_waypoint),
            Parallel("ScanAndLoiter", success_policy="all", failure_policy="one", children=[
                Action("ScanArea", action=_scan_area),
                Action("Loiter", action=_loiter_at_position),
            ]),
            Selector("ThreatResponse", children=[
                Sequence("TrackIfThreat", children=[
...
def build_search_and_track_tree(search_waypoints: List[Dict]) -> BehaviorTree:
    ...
    Structure:
      ...
      └── Action(RTB)
    """
    root = Selector("SearchAndTrack", children=[
        Sequence("TrackMode", children=[
            Condition("ThreatDetected", predicate=_check_threats),
            Action("TrackTarget", action=_track_target),
            Action("ReportPosition", action=_report_position),
        ]),
        Sequence("SearchMode", children=[
            Condition("BatteryOK", predicate=_check_battery),
            Action("ScanArea", action=_scan_area),
...
def build_rtb_tree() -> BehaviorTree:
    ...
    root = Sequence("RTBSequence", children=[
        Action("ReturnToBase", action=_return_to_base),
        Action("ReportPosition", action=_report_position),
    ])
