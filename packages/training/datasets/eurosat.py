...
RGB_URL     = "https://madm.dfki.de/files/sentinel/EuroSAT.zip"
MS_URL      = "https://madm.dfki.de/files/sentinel/EuroSATallBands.zip"
...
def ensure_downloaded(variant: str = "rgb", force: bool = False) -> Path:
    ...
    if variant == "rgb":
        url, root = RGB_URL, RGB_ROOT
    elif variant in ("ms", "multispectral", "all_bands"):
        url, root = MS_URL, MS_ROOT
    else:
    ...
    logger.info("[eurosat] downloading %s (variant=%s)", url, variant)
    try:
