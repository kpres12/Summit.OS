...
class ThreatAssessmentResult(BaseModel):
    ...
    severity_score: float = Field(
        ge=0.0, le=1.0, description="Normalized severity score"
    )
    ...
    estimated_containment_window: Optional[float] = Field(
        None, description="Minutes before escalation needed"
    )
...
class GenericThreatAssessor(BaseThreatAssessor):
    ...
    async def assess_threat(
        ...
        # Simple scoring
        severity_score = min(
            1.0, (intensity * 0.4 + size_metric * 0.3 + spread_rate * 0.3)
        )
        ...
        escalation_reason = (
            f"Severity score {severity_score:.2f} exceeds threshold"
    ...
    def get_escalation_thresholds(self) -> Dict[str, float]:
        ...
        return {"tier_1_to_2": 0.3, "tier_2_to_3": 0.6, "tier_3_to_4": 0.8}
