...
class MAVLinkAdapter(BaseAdapter):
    ...
    def _build_observation(self, sysid: int, state: dict) -> dict:
        ...
        mode_string = _ARDUCOPTER_MODES.get(custom_mode, f"MODE_{custom_mode}") \
            if custom_mode is not None else "UNKNOWN"

        # System status string
        sys_status_int = state.get("system_status")
        system_status_string = _SYS_STATUS.get(sys_status_int, f"STATUS_{sys_status_int}") \
            if sys_status_int is not None else "UNKNOWN"
    ...
    async def send_command(self, entity_id: str, command: dict) -> None:
        ...
        if cmd_type == "GOTO":
            ...
            self._log.info("Sent GOTO to sysid=%d: lat=%.6f lon=%.6f alt=%.1f", sysid, lat, lon, alt)
            ...
            self._log.info("Sent SET_MODE=%s (%d) to sysid=%d", mode_str, custom_mode_int, sysid)
