...
class CyberSyneticEngine:
    ...
    async def process_feedback(self, event: FeedbackEvent) -> None:
        ...
        if event.event_type in _ASSET_EVENTS and event.entity_id:
            try:
                ...
                logger.warning("Asset model update failed for event %s: %s", event.event_id, exc)
        ...
        if event.alert_id and event.event_type in _ALERT_OUTCOME_MAP:
            ...
            try:
                ...
                logger.warning("Alert scorer update failed for event %s: %s", event.event_id, exc)
        ...
        if event.mission_id and event.event_type in _MISSION_TERMINAL:
            ...
            try:
                ...
                logger.warning("Pattern library update failed for event %s: %s", event.event_id, exc)

        logger.debug("Processed feedback event %s (%s)", event.event_id, event.event_type)
    ...
    async def recommend_assets(
        ...
        """
        required_range_m      = float(mission.get("required_range_m")      or
                                      mission.get("range_m")                or 1000.0)
        required_endurance_s  = float(mission.get("required_endurance_s")   or
                                      mission.get("endurance_s")             or 600.0)
    ...
    async def suggest_mission_templates(
        ...
        context_tags = (
            context.get("domain_tags") or
            context.get("tags") or
            ([context["domain"]] if context.get("domain") else [])
        )
    ...
    async def get_system_intelligence(self) -> dict[str, Any]:
        ...
        return {
            ...
            "asset_summary": [
                ...
                for e in sorted(all_estimates, key=lambda e: e.reliability_score, reverse=True)[:10]
            ],
        }
