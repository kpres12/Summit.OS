        ...
        try:
            async with self._async_session.post(url, json=telemetry) as response:
                if response.status == 200:
                    ...
                    raise SummitOSError(
                        f"Failed to publish telemetry: {response.status} - {error_text}"
                    )
        ...
        try:
            async with self._async_session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise SummitOSError(
                        f"Failed to get telemetry: {response.status} - {error_text}"
                    )
        except aiohttp.ClientError as e:
            raise ConnectionError(f"Failed to connect to Summit.OS: {e}")
        ...
        try:
            async with self._async_session.post(url, json=alert) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise SummitOSError(
                        f"Failed to publish alert: {response.status} - {error_text}"
                    )
        except aiohttp.ClientError as e:
            raise ConnectionError(f"Failed to connect to Summit.OS: {e}")
        ...
        try:
            async with self._async_session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise SummitOSError(
                        f"Failed to get alerts: {response.status} - {error_text}"
                    )
        except aiohttp.ClientError as e:
            raise ConnectionError(f"Failed to connect to Summit.OS: {e}")
        ...
        try:
            async with self._async_session.post(url, json=data) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise SummitOSError(
                        f"Failed to acknowledge alert: {response.status} - {error_text}"
                    )
        except aiohttp.ClientError as e:
            raise ConnectionError(f"Failed to connect to Summit.OS: {e}")
        ...
        try:
            async with self._async_session.post(url, json=mission) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise SummitOSError(
                        f"Failed to create mission: {response.status} - {error_text}"
                    )
        except aiohttp.ClientError as e:
            raise ConnectionError(f"Failed to connect to Summit.OS: {e}")
        ...
        try:
            async with self._async_session.get(url) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise SummitOSError(
                        f"Failed to get mission: {response.status} - {error_text}"
                    )
        except aiohttp.ClientError as e:
            raise ConnectionError(f"Failed to connect to Summit.OS: {e}")
        ...
        try:
            async with self._async_session.put(url, json=updates) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise SummitOSError(
                        f"Failed to update mission: {response.status} - {error_text}"
                    )
        except aiohttp.ClientError as e:
            raise ConnectionError(f"Failed to connect to Summit.OS: {e}")
        ...
        try:
            async with self._async_session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise SummitOSError(
                        f"Failed to get intelligence alerts: {response.status} - {error_text}"
                    )
        except aiohttp.ClientError as e:
            raise ConnectionError(f"Failed to connect to Summit.OS: {e}")

    async def get_risk_assessment(
        self, location: Dict[str, float], radius: float = 5000.0
    ) -> Dict[str, Any]:
        """
        ...
        try:
            async with self._async_session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise SummitOSError(
                        f"Failed to get risk assessment: {response.status} - {error_text}"
                    )
        except aiohttp.ClientError as e:
            raise ConnectionError(f"Failed to connect to Summit.OS: {e}")
        ...
        try:
            async with self._async_session.get(url) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise SummitOSError(
                        f"Failed to get world model: {response.status} - {error_text}"
                    )
        except aiohttp.ClientError as e:
            raise ConnectionError(f"Failed to connect to Summit.OS: {e}")
