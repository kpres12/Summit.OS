...
def generate_samples(n: int, seed: int = 42):
    ...
    for _ in range(n):
        ...
        battery         = float(rng.beta(4, 2))             # skewed high — operators charge before dispatch
...
def train(n_samples: int = 40_000):
    ...
    smoke_cases = [
        ...
        ("DELIVER + VTOL + full battery + op approved",
         [MISSION_DELIVER, ASSET_VTOL,       0.95, 0.10, 0.90, 0.33, 0.10, 0.95, 0.0,  0.0, 1.0, 1, 0, 0.70, 1]),
        ("SEARCH + low battery + poor visibility",
         [MISSION_SEARCH,  ASSET_VTOL,       0.12, 0.20, 0.80, 0.66, 0.20, 0.20, 0.0,  0.0, 1.0, 1, 0, 0.60, 0]),
        ("ORBIT + FIXED_WING + good weather",
         [MISSION_ORBIT,   ASSET_FIXED_WING, 0.80, 0.20, 0.88, 0.33, 0.15, 0.90, 0.0,  0.0, 1.0, 1, 0, 0.50, 1]),
        ("QUADCOPTER + high wind",
         [MISSION_MONITOR, ASSET_QUADCOPTER, 0.75, 0.15, 0.85, 0.33, 0.80, 0.80, 0.0,  0.0, 1.0, 1, 0, 0.50, 0]),
        ("SEARCH + night + no op approval",
         [MISSION_SEARCH,  ASSET_VTOL,       0.70, 0.25, 0.75, 0.66, 0.20, 0.70, 0.1, -0.7,-0.7, 0, 1, 0.30, 0]),
        ("INSPECT + good conditions + backup",
         [MISSION_INSPECT, ASSET_QUADCOPTER, 0.85, 0.08, 0.92, 0.33, 0.10, 0.95, 0.0,  0.5, 0.9, 1, 0, 0.80, 1]),
        ("DELIVER + GROUND_UGV + mountainous",
         [MISSION_DELIVER, ASSET_GROUND_UGV, 0.90, 0.30, 0.80, 0.66, 0.10, 0.85, 0.9,  0.0, 1.0, 1, 0, 0.40, 0]),
        ("SURVEY + normal conditions",
         [MISSION_SURVEY,  ASSET_VTOL,       0.75, 0.20, 0.85, 0.33, 0.20, 0.85, 0.2,  0.3, 0.9, 1, 0, 0.60, 1]),
        ("Critical risk + poor visibility",
         [MISSION_SEARCH,  ASSET_FIXED_WING, 0.70, 0.30, 0.90, 1.00, 0.60, 0.25, 0.3,  0.0, 1.0, 1, 0, 0.50, 1]),
        ("Auto-generated, no backup, very low battery",
         [MISSION_PERIMETER, ASSET_QUADCOPTER, 0.10, 0.40, 0.70, 0.33, 0.25, 0.80, 0.0, 0.0, 1.0, 0, 1, 0.20, 0]),
    ]
