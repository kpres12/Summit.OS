...
_SCENARIOS = [
    # ── Wildfire / fire ──────────────────────────────────────────────────────
    (["smoke", "smoke plume", "smoke column", "wildfire smoke", "fire smoke"], 0, (0,0,1,3), 0.5, 1.0),
    (["fire", "active fire", "fire front", "open flame", "wildfire"], 0, (0,0,0,4), 0.6, 1.0),
    (["ember shower", "ember cast", "fire ember"], 0, (0,0,1,3), 0.55, 1.0),
    (["hotspot", "thermal hotspot", "heat anomaly"], 4, (0,1,2,1), 0.5, 1.0),
    (["burning vehicle", "burning structure", "burning building"], 3, (0,0,1,3), 0.6, 1.0),

    # ── Flood / water ────────────────────────────────────────────────────────
    (["flood", "flash flood", "flooding"], 0, (0,0,1,3), 0.5, 1.0),
    (["rising water", "storm surge", "coastal inundation"], 3, (0,0,1,3), 0.55, 1.0),
    (["submerged vehicle", "submerged road", "submerged structure"], 0, (0,1,2,1), 0.5, 0.95),
    ...
    (["missing person", "lost hiker", "missing hiker", "overdue hiker"], 2, (0,0,2,2), 0.4, 0.95),
    (["stranded person", "stranded hiker", "stranded climber"], 2, (0,0,1,3), 0.5, 1.0),
    (["distress signal", "sos signal", "signal mirror", "emergency beacon"], 2, (0,0,1,3), 0.45, 1.0),
    (["survivor", "injured survivor", "avalanche survivor"], 1, (0,0,1,3), 0.5, 1.0),
    (["body", "human remains", "casualty"], 1, (0,1,2,1), 0.4, 0.95),

    # ── Maritime SAR ─────────────────────────────────────────────────────────
    (["overboard person", "man overboard", "swimmer in distress"], 2, (0,0,1,3), 0.5, 1.0),
    (["disabled vessel", "sinking vessel", "sinking boat", "capsized boat"], 2, (0,0,1,3), 0.5, 1.0),
    ...
    (["collapsed building", "building collapse", "structure collapse"], 2, (0,0,1,3), 0.5, 1.0),
    (["earthquake damage", "seismic damage", "rubble field"], 0, (0,1,2,1), 0.5, 1.0),
    ...
    (["radiation source", "radioactive leak", "nuclear incident"], 3, (0,0,0,4), 0.4, 1.0),

    # ── Infrastructure inspection ─────────────────────────────────────────────
    (["power line damage", "downed power line", "damaged pylon"], 6, (0,1,2,1), 0.5, 1.0),
    (["pipeline leak", "pipeline damage", "pipe corrosion"], 6, (0,1,2,1), 0.45, 1.0),
    (["solar panel", "solar farm inspection", "solar array"], 6, (1,2,1,0), 0.5, 1.0),
    (["wind turbine", "wind turbine inspection", "blade damage"], 6, (1,2,1,0), 0.5, 1.0),
    ...
    (["irrigation leak", "irrigation failure", "dry field"], 6, (1,2,1,0), 0.5, 0.95),
    (["field survey", "crop survey", "farmland survey"], 0, (2,2,0,0), 0.5, 1.0),
    (["livestock", "cattle herd", "sheep flock"], 1, (2,1,1,0), 0.5, 0.95),
    (["illegal crop", "illicit cultivation", "illegal farm"], 0, (0,1,2,1), 0.5, 1.0),

    # ── Wildlife / environmental ──────────────────────────────────────────────
    (["wildlife", "wild animal", "animal"], 1, (2,2,0,0), 0.5, 0.95),
    (["poaching activity", "illegal poaching", "wildlife poaching"], 3, (0,1,2,1), 0.5, 1.0),
    ...
    (["medical emergency", "person down", "unresponsive person"], 1, (0,0,1,3), 0.5, 1.0),
    (["trauma scene", "accident scene", "crash scene"], 0, (0,0,1,3), 0.5, 1.0),
    (["triage area", "casualty collection", "aid station"], 1, (0,1,2,1), 0.5, 0.95),

    # ── Security / intrusion ──────────────────────────────────────────────────
    (["perimeter breach", "fence breach", "intrusion detected"], 3, (0,1,2,1), 0.5, 1.0),
    (["unauthorized uav", "rogue drone", "suspicious drone"], 4, (0,0,1,3), 0.5, 1.0),
    (["armed individual", "weapon detected", "hostile individual"], 3, (0,0,0,4), 0.5, 1.0),
    ...
    (["orbit target", "loiter pattern", "persistent surveillance"], 4, (0,1,2,1), 0.6, 1.0),
    (["thermal orbit", "isr orbit", "continuous watch"], 4, (0,1,2,1), 0.6, 1.0),
    (["vessel tracking", "ship tracking", "maritime surveillance"], 4, (1,2,1,0), 0.5, 1.0),
    ...
    (["aid delivery", "medical supply delivery", "food supply delivery"], 5, (0,1,2,1), 0.7, 1.0),
    (["package delivery", "cargo drop", "payload delivery"], 5, (2,2,0,0), 0.7, 1.0),
    (["logistics mission", "resupply run", "transport mission"], 5, (2,2,0,0), 0.7, 1.0),
    (["humanitarian delivery", "emergency supply drop", "relief delivery"], 5, (0,1,2,1), 0.7, 1.0),
    (["courier mission", "last-mile delivery", "package drop"], 5, (2,2,0,0), 0.7, 1.0),

    # ── Additional INSPECT scenarios ─────────────────────────────────────────
    (["infrastructure inspection", "asset inspection", "close inspection"], 6, (1,2,1,0), 0.6, 1.0),
    (["roof inspection", "facade inspection", "structure inspection"], 6, (1,2,1,0), 0.55, 1.0),
    (["bridge inspection", "road inspection", "rail inspection"], 6, (0,1,2,1), 0.55, 1.0),
    (["photovoltaic inspection", "panel inspection", "cable inspection"], 6, (1,2,1,0), 0.6, 1.0),

    # ── Hurricane / severe weather ────────────────────────────────────────────
    (["tornado", "tornado touchdown", "twister"], 3, (0,0,0,4), 0.5, 1.0),
    (["hurricane damage", "cyclone damage", "typhoon damage"], 0, (0,1,2,1), 0.5, 1.0),
...
def generate_mission_samples(n: int = 8000) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for _ in range(n // 10):
        ...
        obs = {"class": f"unknown_{_RNG.randint(0,999)}", "confidence": conf,
               "lat": _RNG.uniform(-60, 70), "lon": _RNG.uniform(-180, 180)}
        X.append(extract(obs))
        y.append(0)  # SURVEY
...
def generate_risk_samples(n: int = 8000) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for _ in range(n // 10):
        ...
        obs = {"class": _RNG.choice(["object", "blob", "movement", "anomaly"]),
               "confidence": conf,
               "lat": _RNG.uniform(-60, 70), "lon": _RNG.uniform(-180, 180)}
        X.append(extract(obs))
        y.append(0)  # LOW
