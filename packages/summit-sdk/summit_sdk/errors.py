...
class TimeoutError(SummitError):
    ...
    def __init__(self, message: str = "Request timed out", timeout: float = 0):
        super().__init__(message, ErrorCode.TIMEOUT, details={"timeout_s": timeout})
...
class ValidationError(SummitError):
    ...
    def __init__(
        self, message: str = "Validation error", fields: Optional[Dict] = None
    ):
        super().__init__(
            message,
...
class ServerError(SummitError):
    ...
    def __init__(self, message: str = "Internal server error", status: int = 500):
        code = (
            ErrorCode.SERVICE_UNAVAILABLE if status == 503 else ErrorCode.SERVER_ERROR
        )
        super().__init__(message, code, status=status)
...
class NotConnectedError(SummitError):
    ...
    def __init__(self):
        super().__init__(
            "Client not connected — call await client.connect() first",
